...
class MCTS:
    ...
    def simulate(self, node, depth=10):
        if depth == 0:
            return 0
    ...
    def run(self, root, actions, simulations=100): 
...
while cap.isOpened() and frame_count < max_frames:
    ...
    with open(gt_file, 'r') as f:
        for line in f:
            ...
            if frame_num == frame_count + 1 and obj_id in ids:
                centers[obj_id] = np.array([[x], [y]])

    for obj_id in centers:
        if obj_id not in trackers:
            trackers[obj_id] = {
                'kf': KalmanFilter(initial_x=centers[obj_id][0][0], initial_y=centers[obj_id][1][0]),
                'wm': WorldModel(),
                'mcts': MCTS(WorldModel()),
                'optimizer': optim.Adam(WorldModel().parameters(), lr=0.001)
            }

        center = centers[obj_id]
        kf = trackers[obj_id]['kf']
        wm = trackers[obj_id]['wm']
        mcts = trackers[obj_id]['mcts']
        optimizer = trackers[obj_id]['optimizer']
        ...
        pred_data.append({'frame': frame_count + 1, 'id': obj_id, 'pred_x': pred_x, 'pred_y': pred_y})
...
cap.release()
print('Multi-object predictions with tuned MCTS saved.')
...
with open(gt_file, 'r') as f:
    for line in f:
        ...
        y = float(parts[3]) + float(parts[5]) / 2
        if obj_id in ids:
            gt_data.append({'frame': frame, 'id': obj_id, 'gt_x': x, 'gt_y': y})

pred_df = pd.DataFrame(pred_data)
gt_df = pd.DataFrame(gt_data)
result = pd.merge(pred_df, gt_df, on=['frame', 'id'])
result['distance'] = np.sqrt((result['pred_x'] - result['gt_x'])**2 + (result['pred_y'] - result['gt_y'])**2)
print(result[['frame', 'id', 'pred_x', 'pred_y', 'gt_x', 'gt_y', 'distance']])
print('Average Error:', result['distance'].mean())
result.to_csv('amarzero_results/evaluation_multi.csv', index=False)
print('Multi-object Evaluation saved to amarzero_results/evaluation_multi.csv')
